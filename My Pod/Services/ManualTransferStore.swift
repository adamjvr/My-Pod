...
final class ManualTransferStore {
    ...
    private let conversionService = ConversionService(maxConcurrent: 1)
    ...
    func addQueued(to device: IPodDevice) async {
...
nonisolated enum ManualTrackLoader {
    ...
    private static func makeTrack(_ url: URL) async -> LibraryTrack? {
        ...
        return LibraryTrack(
            ...
            durationMS: needsConversion ? (probe?.durationMS ?? FLACHeader.durationMS(of: url)) : 0
        )
    }
